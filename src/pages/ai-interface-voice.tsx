...
export default function AIInterfaceVoice() {
            ...
            if (msg.type === "meta") {
                ...
                if (!routeConversationId) {
                    window.history.replaceState(
                        null,
                        "",
                        `/ai-interface-voice/c/${msg.conversation_id}`
                    );
                }
    ...
    const playAudioChunk = async (base64Chunk: string, done = false) => {
        ...
        // accumulate chunks
        pendingAudioChunksRef.current.push(base64Chunk);

        // if sentence not done, wait
        if (!done) return;

        // combine chunks into one ArrayBuffer
        const combined = pendingAudioChunksRef.current
            .map(chunk => Uint8Array.from(atob(chunk), c => c.charCodeAt(0)))
            .reduce((acc, curr) => {
                const tmp = new Uint8Array(acc.length + curr.length);
                tmp.set(acc, 0);
                tmp.set(curr, acc.length);
                return tmp;
            }, new Uint8Array());

        pendingAudioChunksRef.current = []; // reset for next sentence
                ...
                <div className="flex w-full h-full">
                    ...
                    >
                        ...
                        <div className="px-2 space-y-1">
                            {[
                                { label: "New Chat", href: '/ai-interface-voice', icon: SquarePen },
                            ...
                            <button
                                onClick={() => { window.location.href = '/ai-interface'; }}
                                className="group relative w-full px-3 py-2 rounded-lg flex items-center gap-3 text-sm
                                    animate-gradient opacity-80"
                            >
                                <MessagesSquare />
                                {/* Text (only when open) */}
                                {isOpen && <span className="truncate dark:text-white">Sega Chat</span>}
                        ...
                        <div className="flex flex-col flex-1 overflow-hidden">
                            ...
                            <div className="flex-1 overflow-y-auto px-2 space-y-1">
                                {chatList.map((chat, index) => (
                                    <button
                                        key={index}
                                        onClick={() => navigate(`/ai-interface-voice/c/${chat.id}`)}
